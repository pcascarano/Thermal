...
image_syntetic=0;           % 1 if GT is available,  0 if GT is not available
etaM=0;                     % 0 suggested
...
% Low resolution acquired
g_image=importdata('./data/Accursio_LR_1.txt'); g_image=g_image - min(min(g_image)); g_image=g_image/max(max(g_image));
